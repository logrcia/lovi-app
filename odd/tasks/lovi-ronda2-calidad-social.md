...
- [~] T14 (FASE E) Lint + TS + build ✅ | smoke rutas sin sesión (8 protegidas 307 -> /auth/login, 3 públicas 200) ✅ | smoke rutas con sesión (feed, users, users/[id], calendar, profile, loviar, pets/new: 200) ✅ | test A/B 2 usuarios + RLS: 25/25 PASS (usuarios lovi-e2e-a/b@lovi.dev creados auto-confirmados; 1 "FAIL" inicial = bug de assertion del script — RPC devuelve tabla, se accedía .followers_count sobre array; corregido, re-run 25/25) ✅ | PENDIENTE: recorrido manual en navegador (parte del usuario). NOTA: notFound() en pets/[id] y users/[id] renderiza bien la UI 404 pero Next 16 devuelve status HTTP 200 por streaming con instant=false (known nuance, no bloquea).
...
- FASE E: T14 en curso — lint/TS/build ✅, smoke rutas sin/con sesión ✅, test A/B 2 usuarios + RLS 25/25 ✅. FIX post-smoke (reportado por usuario en /users): `components/follow-button.tsx` pasaba un closure inline como form action (`action={async () => ...}`) — Next 16 no serializa closures de server components: error "Functions cannot be passed directly to Client Components" + caída de la página (sin searchbar). Corregido con `action={toggleFollow.bind(null, targetUserId)}` y `toggleFollow` pasó a devolver `Promise<void>` (era Promise<ActionResult> que el form action no acepta y nadie consumía). tsc ✅, lint 0 errores ✅, smoke /users y /users/[id] 200 con searchbar y follow button ✅ PENDIENTE: recorrido manual en navegador (checklist entregado al usuario), limpieza usuarios de prueba en dashboard (lovi-test-a/b@lovi.dev bloqueados; lovi-e2e-a/b@lovi.dev confirmados reutilizables) y mantener lovi-rls-test.mjs como regression.
